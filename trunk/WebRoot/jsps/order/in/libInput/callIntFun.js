/**
 * called in internal files in libInput
 * @returns
 */
function callIntFun(){
	document.writeln("<script type='text/javascript' src='addVar.js'></script>");
	document.writeln("<script type='text/javascript' src='forAddFun.js'></script>");
	document.writeln("<script type='text/javascript' src='addFunSecond.js'></script>");
	document.writeln("<script type='text/javascript' src='goodsTypeFinal.js'></script>");
	document.writeln("<script type='text/javascript' src='goodsTypeVar.js'></script>");	
	document.writeln("<script type='text/javascript' src='goodsTypeVar2.js'></script>");
	document.writeln("<script type='text/javascript' src='pricesVar.js'></script>");
	document.writeln("<script type='text/javascript' src='pricesReplaceFun1.js'></script>");
	document.writeln("<script type='text/javascript' src='pricesReplaceFun2.js'></script>");
	document.writeln("<script type='text/javascript' src='supplierForFun.js'></script>");
	document.writeln("<script type='text/javascript' src='addFunJquery.js'></script>");


}